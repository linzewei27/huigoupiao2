...
function initEvent() {
    $("body").on("click",".buy-btn",function () {
        ...
        $("#mainIframe",parent.document).attr("src","http://www.hgp.com/huigoupiao/Theater/Index/details.html?movieid="+movieid+"&theaterid="+theaterid);
    });
    $("body").on("click",".selecttime li,.place li",function () {
        ...
        $.ajax({
            ...
            data:{mid:mid,time:time,place:place},
            type:"post",
...
function setTheaterInfo(theaterinfo,bookInfo,isFirst) {
    ...
    for(var i=0;i<bookInfo.length;i++){
        ...
        cinemasList+= '<div class="cinema-cell">\n' +
            ...
            '                    <p class="cinema-address">地址：'+ theaterinfo[i].place+'</p>\n' +
            '                </div>\n' +
            '\n' +
            '                <div class="buy-btn" value="'+bookInfo[i].movieid+'" theaterid="'+theaterinfo[i].id +'">\n' +
            '                    <a >选座购票</a>\n' +
            '                </div>\n' +
            '\n' +
            '                <div class="price">\n' +
